...
public class RegisterViewController extends ViewController implements Initializable {
    ...
    public void registerAction() {
        ...
        if (result.isSuccessful()) {
            result = null;
            Client.client.authorization(email);

            TextInputDialog dialog = new TextInputDialog();
            dialog.setTitle("authorization");
            dialog.setHeaderText("write the 6 digits number");

            Optional<String> resultDialog = dialog.showAndWait();

            resultDialog.ifPresent(text -> {
                Client.client.checkCode(text);
                waitForResponse();
                if (result.isSuccessful()) {
                    Client.client.addUser(username, password, nickName, email);
                    throwAlert("Registered Successfully!", "Dear \"" + nickName + "\" welcome to the Gwent World!");
                    toggle();
                    registerButton.setVisible(false);
                } else {
                    throwAlert("WRONG", "please enter the correct code");
                }
            });
    ...
    public void done(ActionEvent actionEvent) {
        if (!confirmAnswer.getText().equals(secA.getText())) {
            throwAlert("no bro", "confirm your answer properly, you can not change it in future");
            return;
        }
        String username = usernameField.getText();
